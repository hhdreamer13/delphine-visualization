...
const SchoolForce = ({ data }) => {
  ...
  const seasons = _.range(1, 11).map((season) => ({
    id: `season-${season}`,
    title: `Season ${season}`,
    group: "season",
  }));
  ...
  const nodesData = data
    .map((d) => {
      return {
        ...
        title: d.title,
        group: d.school,
        season: d.season, // Add this line
      };
    })
    .concat(schools)
    .concat(seasons); // Add seasons to nodesData
  ...
  const linksData = data
    .map((d) => {
      return [
        ...
        {
          source: `data-${d.id}`,
          target: seasons.find((s) => s.id === `season-${d.season}`).id,
        },
      ];
    })
    .flat();
  ...
  const nodeFill = (d) => {
    if (d.group === "school") {
      return colorScale(schools.find((school) => school.id === d.id).title);
    }
    if (d.group === "season") {
      return "#999";
    }
  ...
  const nodeStrokeWidth = (d) => {
    if (!d) return 1.5; // Add this line to handle undefined objects
    if (d.group === "school" || d.group === "season") return 2;
    return 1.5;
  };
  const nodeStrokeOpacity = 1; // node stroke opacity
  const nodeRadius = (d) => {
    if (d.group === "school") return 10;
    if (d.group === "season") return 7;
    return 5;
  };
